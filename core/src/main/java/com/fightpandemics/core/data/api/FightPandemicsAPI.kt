...
interface FightPandemicsAPI {
    ...
    suspend fun updatePost(@Path("postId") postId: String, @Body postRequest: PostRequest)

    @PUT("/api/posts/{postId}/likes/{userId}")
    suspend fun updatePost(@Path("postId") postId: String, @Path("userId") userId: String)
